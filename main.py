...
k, T, C, L = symbols('k T C L') #Сосед поменял местами буквы T и C # Что это означает? (Это объявление переменных, которые будут использоваться в формулах в дальнейшем) - оценка 5

# 1 способ
C_ost = 100000
Am_lst = [] # Что это означает? (Это список, в который будут добавляться значения переменной Am) - оценка 5
C_ost_lst = []

for i in range(5):
  Am = (C - L) / T
  C_ost -= Am.subs({L: 0, T: 4, C: 100000}) #Сосед поменял значение переменной T с 5 на 4
  Am_lst.append(round(Am.subs({L: 0, T: 4, C: 100000}), 4)) #Сосед поменял значение переменной T с 5 на 4, также поменял последнее значение с 2 на 4
  C_ost_lst.append(round(C_ost, 4)) #Сосед поменял значение переменной T с 5 на 4
